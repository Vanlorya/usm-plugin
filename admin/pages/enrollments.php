...
<div class="wrap usm-wrap">
	...
	<?php else : ?>
		...
		$total = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$table} e LEFT JOIN {$wpdb->prefix}usm_students s ON e.student_id = s.id WHERE 1=1 {$where_status} {$where_pay} {$where_search}" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.PreparedSQL
		...
		$items = $wpdb->get_results( $wpdb->prepare(
			"SELECT e.*, s.full_name AS student_name, p.name AS package_name
			 ...
			 LEFT JOIN {$wpdb->prefix}usm_students s ON e.student_id = s.id
			 LEFT JOIN {$wpdb->prefix}usm_packages p ON e.package_id = p.id
			 WHERE 1=1 {$where_status} {$where_pay} {$where_search}
						...
						<tr>
							...
							<td class="usm-actions">
								<?php if ( 'active' === $row->status ) : ?>
									<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=usm-enrollments&action=pause&id=' . $row->id ), 'usm_pause_enrollment_' . $row->id ) ); ?>">
										<?php esc_html_e( 'Tạm hoãn', 'usm' ); ?>
									</a>
								<?php endif; ?>
								<?php if ( 'paused' === $row->status ) : ?>
									<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=usm-enrollments&action=resume&id=' . $row->id ), 'usm_resume_enrollment_' . $row->id ) ); ?>">
										<?php esc_html_e( 'Kích hoạt', 'usm' ); ?>
									</a>
								<?php endif; ?>
								<?php if ( in_array( $row->status, array( 'completed', 'expired' ), true ) ) : ?>
									<a href="<?php echo esc_url( admin_url( 'admin.php?page=usm-enrollments&action=add&renew_student=' . $row->student_id . '&renew_package=' . $row->package_id ) ); ?>" style="color:#2271b1; font-weight:600;">
										<?php esc_html_e( '🔄 Gia hạn', 'usm' ); ?>
									</a>
								<?php endif; ?>
								<?php if ( 'paid' !== $row->payment_status ) : ?>
									<a href="<?php echo esc_url( admin_url( 'admin.php?page=usm-enrollments&action=payment&id=' . $row->id ) ); ?>" style="color:#00a32a; font-weight:600;">
										<?php esc_html_e( '💰 Thu tiền', 'usm' ); ?>
									</a>
								<?php endif; ?>
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=usm-enrollments&action=print&id=' . $row->id ) ); ?>" target="_blank" style="font-size:11px;" title="In phiếu thu">
									🖨️
								</a>
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=usm-enrollments&action=payment&id=' . $row->id ) ); ?>" style="font-size:11px;">
									<?php esc_html_e( '📋 LS', 'usm' ); ?>
								</a>
								<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=usm-enrollments&action=delete&id=' . $row->id ), 'usm_delete_enrollment_' . $row->id ) ); ?>" class="usm-confirm-delete" style="color:#d63638;">
